    ...
    public class Utils
    {
        ...
        public static int GetInArrayID(string strSearch, string[] stringArray, bool caseInsensetive)
        {
            for (int i = 0; i < stringArray.Length; i++)
            {
                if (caseInsensetive)
                {
                    if (strSearch.ToLower(CultureInfo.InvariantCulture) == stringArray[i].ToLower(CultureInfo.InvariantCulture))
                    {
                        return i;
                    }
                }
        ...
        {
            if (strContent.IndexOf(strSplit, StringComparison.InvariantCulture) < 0)
            {
        ...
        public static void ResponseFile(string filepath, string filename, string filetype)
        {
            ...
            try
            {
                ...
                HttpContext.Current.Response.AddHeader("Content-Disposition", "attachment;filename=" + Utils.UrlEncode(filename.Trim().Replace(' ', '_')).Replace("+", " "));
        ...
        public static bool IsImgFilename(string filename)
        {
            ...
            if (filename.EndsWith(".", StringComparison.Ordinal) || filename.IndexOf(".", StringComparison.Ordinal) == -1)
            {
                return false;
            }
            string extname = filename.Substring(filename.LastIndexOf(".", StringComparison.Ordinal) + 1).ToLower(CultureInfo.InvariantCulture);
        ...
        public static string MD5(string str)
        {
            ...
            {
                ret.AppendFormat(CultureInfo.InvariantCulture, "{0:x2}", b[i]);
            }
            return ret.ToString();
        }
        ...
        public static string CleanSafeSqlString(string str)
        {

            return Regex.Replace(str, @"[-|;|,|\/|\(|\)|\[|\]|\}|\{|%|@|\*|!|\']","");
        }

        public static string CleanSafeSqlString2(string str)
        {
        ...
        /// <param name="basePath">基地址（比如虚拟目录的地址：/QueryReport）</param>
        ...
        /// <summary>
        /// 不要超过60个数组。不能重复。 输入 整数。得到 2的开每个整数总和
        /// </summary>
        /// <param name="array"></param>
        /// <returns></returns>
        public static long getSum2N(int[] array)
        {
            long result = 0;
            int count = array.Length;
            for (int i = 0; i < count; i++)
            {
                result += (long)(Math.Pow(2, array[i]));
            }
            return result;
        }


        public static int[] getArray2N(long sum)
        {
            List<int> array = new List<int>();
            while (sum > 0)
            {
                int intr = getClose2N(sum);
                array.Add(intr);
                sum = sum - (long)Math.Pow(2, intr);
            }
            return array.ToArray();
        }

        public static List<int> getList2N(long sum)
        {
            List<int> array = new List<int>();
            while (sum > 0)
            {
        ...
        private static int getClose2N(long sum)
        {
            int result = 0;
            result = (int)Math.Floor(Math.Log(sum, 2));
            return result;
        }

        public static int[] intOrder(int[] Result)
        {
        ...
        {
            public static bool CreateFloder(string path,out string msg)
            {
                bool res = true;
                msg = "";
                try
